...
useSeoMeta({
    ...
    description:
        "Volg je PMO-vrije streak dag voor dag. Volledig privé, geen account, geen server.",
    ogTitle: "Gelukt — PMO Recovery Tracker",
    ogDescription:
        "Persoonlijke recovery tracker. Privé, geen account, geen server.",
    ogType: "website",
...
const heeftGecheckt = computed(() =>
    drangmomenten.value.length > 0 || streakData.value.terugvallen.length > 0
);
...
function haalTriggerId() {
    return gekozenTriggerId.value === "andere"
        ? undefined
        : gekozenTriggerId.value || undefined;
}

function haalTriggerNaam() {
    if (gekozenTriggerId.value === "andere")
        return andereTrigger.value.trim() || undefined;
    return triggerLijst.value.find((t) => t.id === gekozenTriggerId.value)
        ?.naam;
}
...
const recenteActiviteit = computed((): ActiviteitItem[] =>
    [
        ...drangmomenten.value.map((d) => ({
            id: d.id,
            type: "drang" as const,
            datumTijd: d.datumTijd,
            intensiteit: d.intensiteit,
            triggerNaam: d.triggerNaam,
            relatieveDatum: relatiefDatum(d.datumTijd),
        })),
        ...streakData.value.terugvallen.map((t) => ({
            id: t.id,
            type: "terugval" as const,
            datumTijd: t.datumTijd,
            triggerNaam: t.triggerNaam,
            relatieveDatum: relatiefDatum(t.datumTijd),
        })),
    ]
        .sort(
            (a, b) =>
                new Date(b.datumTijd).getTime() -
                new Date(a.datumTijd).getTime(),
        )
        .slice(0, 5),
);
</script>
...
<template>
    <div>
        ...
        <section class="g-infobox mb-8">
            <p class="font-bijbel text-lg leading-relaxed">
                {{ dagelijksTekst.tekst }}
            </p>
        ...
        <section class="mb-8">
            <div class="text-7xl font-bold tabular-nums leading-none g-kleur-streak">
                {{ aantalDagen }}
            </div>
        ...
        <section>
            ...
            <template v-if="recenteActiviteit.length > 0">
                <div
                    v-for="item in recenteActiviteit"
                    :key="item.id"
                    class="flex items-start gap-2 mb-3"
                >
                    <TrendingUp
                        v-if="item.type === 'drang'"
                        :size="14"
                        class="g-kleur-drang mt-0.5 shrink-0"
                    />
                    <X
                        v-else
                        :size="14"
                        class="g-kleur-gevallen mt-0.5 shrink-0"
                    />
                    <div>
                        <p class="g-body">
                            <span
                                v-if="item.type === 'drang'"
                                class="g-kleur-drang"
                                >drang {{ item.intensiteit }}/10</span
                            >
                            <span v-else class="g-kleur-gevallen"
                                >gevallen</span
                            >
                            <span class="text-stone-500">
                                — {{ item.relatieveDatum }}</span
                            >
                        </p>
                        <p v-if="item.triggerNaam" class="g-meta">
                            {{ item.triggerNaam }}
                        </p>
                    </div>
                </div>
                    ...
                    <div class="w-full max-w-sm">
                        ...
                        <template v-if="stap === 'keuze'">
                            ...
                            <div class="flex flex-col gap-3 mb-8">
                                ...
                                >
                                    drang gevoeld
                                    <div class="text-xs text-orange-300 mt-0.5">
                                        maar niet gevallen
                                    </div>
                                </button>
                                <button
                            ...
                            <button @click="reset" class="g-link">
                                annuleren
                            </button>
                        </template>
                        ...
                        <template v-else-if="stap === 'drang'">
                            ...
                            <div class="g-infobox mb-6">
                                <p class="font-bijbel text-sm leading-relaxed">
                                    {{ dagelijksTekst.tekst }}
                                </p>
                                <p class="g-meta mt-1">
                                    — {{ dagelijksTekst.referentie }} (HSV)
                                </p>
                            </div>
                            <div class="mb-6">
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <button
                                        ...
                                        :class="
                                            intensiteit === n
                                                ? 'bg-black text-white dark:bg-paper dark:text-black'
                                                : ''
                                        "
                                    >
                                        {{ n }}
                                ...
                                <div class="flex justify-between g-meta mt-1">
                                    <span>licht</span><span>zwaar</span>
                                </div>
                            </div>
                            <div class="mb-6">
                                <label class="g-label"
                                    >trigger (optioneel)</label
                                >
                                <div
                                    v-for="t in triggerLijst"
                                    :key="t.id"
                                    class="mb-2"
                                >
                                    <label
                                        class="flex items-start gap-2 g-body cursor-pointer"
                                    >
                                        <input
                                            type="radio"
                                            :value="t.id"
                                            v-model="gekozenTriggerId"
                                            class="mt-0.5 accent-black dark:accent-paper"
                                        />
                                        {{ t.naam }}
                                    </label>
                                </div>
                                ...
                                <input
                                    v-if="gekozenTriggerId === 'andere'"
                                    v-model="andereTrigger"
                                    type="text"
                            ...
                            <div class="flex gap-3">
                                <button
                                    @click="slaaDrangOp"
                                    :disabled="!intensiteit"
                                    class="g-btn-solid"
                                >
                                    opslaan
                                </button>
                                <button @click="stap = 'keuze'" class="g-btn">
                                    terug
                                </button>
                            </div>
                        </template>
                        ...
                        <template v-else-if="stap === 'gevallen'">
                            ...
                            <div class="mb-8">
                                <label class="g-label"
                                    >notitie (optioneel)</label
                                >
                                <textarea
                                    v-model="notitie"
                                    rows="3"
                                    placeholder="wat ging er mis, of wat wil je onthouden?"
                                    class="g-textarea"
                                />
                            </div>
                            <div class="flex gap-3">
                                <button
                                    @click="bevestigTerugval"
                                    class="g-btn-solid"
                                >
                                    bevestigen
                                </button>
