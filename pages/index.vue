...
const { succesvolleDagen, logSucces } = useSucces();
const { dagelijksTekst } = useBijbeltekst();

const heeftGecheckt = computed(() =>
  drangmomenten.value.length > 0 ||
  streakData.value.terugvallen.length > 0 ||
  succesvolleDagen.value.length > 0
);

type Stap = "gesloten" | "keuze" | "drang" | "gevallen" | "succes";
const stap = ref<Stap>("gesloten");
const intensiteit = ref<number | null>(null);
const gekozenTriggerId = ref("");
const andereTrigger = ref("");
const notitie = ref("");
const gekozenDatumTijd = ref("");

function nuAlsDatumTijdInput(): string {
  const nu = new Date();
  const pad = (n: number) => String(n).padStart(2, "0");
  return `${nu.getFullYear()}-${pad(nu.getMonth() + 1)}-${pad(nu.getDate())}T${pad(nu.getHours())}:${pad(nu.getMinutes())}`;
}

function kiesStap(s: "drang" | "gevallen" | "succes") {
  gekozenDatumTijd.value = nuAlsDatumTijdInput();
  stap.value = s;
}

function datumTijdNaarISO(): string {
  return gekozenDatumTijd.value
    ? new Date(gekozenDatumTijd.value).toISOString()
...
async function slaaDrangOp() {
  ...
  const drang = { ...{ intensiteit: intensiteit.value }, datumTijd: datumTijdNaarISO(), triggerId: tid, triggerNaam: haalTriggerNaam() };
  await logDrang(drang.intensiteit, drang.triggerId, drang.triggerNaam, drang.datumTijd);
...
async function slaSuccesOp() {
  await logSucces(datumTijdNaarISO(), notitie.value.trim() || undefined);
  reset();
}
...
const recenteActiviteit = computed((): ActiviteitItem[] =>
  [
    ...
    ...succesvolleDagen.value.map((s) => ({
      id: s.id, type: "succes" as const, datumTijd: s.datumTijd,
      relatieveDatum: relatiefDatum(s.datumTijd),
    })),
  ]
...
// Gedeelde trigger-sectie template helper
const triggerSectie = computed(() => triggerLijst.value);
</script>
...
<template>
  <div>
    ...
    <section class="g-infobox mb-8">
      <p class="font-bijbel text-lg leading-relaxed">{{ dagelijksTekst.tekst }}</p>
    ...
    <section class="mb-8">
      ...
      <p v-if="heeftGecheckt" class="g-meta mt-1 mb-6">gestart {{ geformatteerdeBeginDatum }}</p>
      <p v-else class="g-meta mt-1 mb-6">check in om te beginnen met tellen</p>
    ...
    <section>
      ...
      <template v-if="recenteActiviteit.length > 0">
        <div v-for="item in recenteActiviteit" :key="item.id" class="flex items-start gap-2 mb-3">
          ...
          <CheckCircle v-else-if="item.type === 'succes'" :size="14" class="g-kleur-streak mt-0.5 shrink-0" />
          ...
          <div>
            <p class="g-body">
              ...
              <span v-else-if="item.type === 'succes'" class="g-kleur-streak">succesvolle dag</span>
      ...
      <p v-else class="g-meta">nog niet ingechecked</p>
    </section>
          ...
          <div class="w-full max-w-sm">
            ...
            <template v-if="stap === 'keuze'">
              ...
              <div class="flex flex-col gap-3 mb-8">
                <button @click="kiesStap('succes')" class="g-btn text-left border-green-600 text-green-700 dark:text-green-500 hover:bg-green-700 hover:text-white hover:border-green-700">
                  succesvolle dag
                  <div class="text-xs text-green-500 dark:text-green-600 mt-0.5">geen drang, niet gevallen</div>
                </button>
                <button @click="kiesStap('drang')" class="g-btn-drang">
                  drang gevoeld
                  <div class="text-xs text-orange-300 mt-0.5">maar niet gevallen</div>
                </button>
                <button @click="kiesStap('gevallen')" class="g-btn-danger">
                  gevallen
            ...
            <!-- Succesvolle dag -->
            <template v-else-if="stap === 'succes'">
              <h2 class="g-heading">succesvolle dag</h2>
              <div class="mb-4">
                <label class="g-label">datum & tijdstip</label>
                <input v-model="gekozenDatumTijd" type="datetime-local" class="g-input" />
              </div>
              <div class="mb-8">
                <label class="g-label">notitie (optioneel)</label>
                <textarea v-model="notitie" rows="3" class="g-textarea" />
              </div>
              <div class="flex gap-3">
                <button @click="slaSuccesOp" class="g-btn-solid">opslaan</button>
            ...
            <template v-else-if="stap === 'drang'">
              ...
              <div class="mb-6">
                ...
                <div v-for="t in triggerSectie" :key="t.id" class="mb-2">
                ...
                <input v-if="gekozenTriggerId === 'andere'" v-model="andereTrigger" type="text"
                  placeholder="beschrijf de trigger" class="g-input mt-2" />
              </div>
              <div class="flex gap-3">
